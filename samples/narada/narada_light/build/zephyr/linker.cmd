...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x0 + 0x0), LENGTH = (704 * 1024 - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x20000000, LENGTH = (144 * 1K)
    FLASH_CCFG ( rw ) : ORIGIN = (720808), LENGTH = (88) SRAM1 ( rw ) : ORIGIN = (285212672), LENGTH = (8192)
...
narada_switch_server_area : ALIGN_WITH_INPUT { _narada_switch_server_list_start = .; KEEP(*(SORT_BY_NAME(._narada_switch_server.static.*))); _narada_switch_server_list_end = .;; } > RAM AT > FLASH
    __data_region_end = .;
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    FLASH_CCFG (NOLOAD) : { __FLASH_CCFG_start = .; KEEP(*(FLASH_CCFG)) KEEP(*(FLASH_CCFG.*)) __FLASH_CCFG_end = .; } > FLASH_CCFG __FLASH_CCFG_size = __FLASH_CCFG_end - __FLASH_CCFG_start; __FLASH_CCFG_load_start = LOADADDR(FLASH_CCFG); SRAM1 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
